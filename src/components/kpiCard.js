// ============================================================
// KPI CARD COMPONENT — with delta/variation support
// ============================================================

import { formatarMoeda } from '../data/businessLogic.js';

const KPI_ICONS = {
  money: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>`,
  down: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="23 18 13.5 8.5 8.5 13.5 1 6"/><polyline points="17 18 23 18 23 12"/></svg>`,
  up: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/><polyline points="16 7 22 7 22 13"/></svg>`,
  check: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>`,
  clock: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>`,
  alert: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>`,
  balance: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 7V4a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v3"/><line x1="12" y1="11" x2="12" y2="17"/><line x1="9" y1="14" x2="15" y2="14"/></svg>`,
  file: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>`,
  percent: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="19" y1="5" x2="5" y2="19"/><circle cx="6.5" cy="6.5" r="2.5"/><circle cx="17.5" cy="17.5" r="2.5"/></svg>`,
  calendar: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>`,
  users: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>`,
  target: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg>`,
};

export function renderKPICard({ label, value, sub, color = 'indigo', icon = 'money', isCurrency = true, delta, deltaLabel }) {
  const displayValue = isCurrency
    ? formatarMoeda(value)
    : (typeof value === 'number' ? value.toLocaleString('pt-BR') : value);
  const deltaHtml = delta !== undefined ? `
    <div class="kpi-delta ${delta >= 0 ? 'kpi-delta-up' : 'kpi-delta-down'}">
      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="${delta >= 0 ? '18 15 12 9 6 15' : '6 9 12 15 18 9'}"/></svg>
      ${Math.abs(delta).toFixed(1)}%${deltaLabel ? ` <span class="kpi-delta-label">${deltaLabel}</span>` : ''}
    </div>
  ` : '';

  const sparklinePath = color === 'red' 
    ? 'M 0 6 C 20 14, 40 8, 55 22 C 65 24, 72 20, 80 28'
    : (color === 'amber'
      ? 'M 0 16 C 20 22, 35 12, 50 18 C 65 14, 72 16, 80 8'
      : 'M 0 24 C 20 18, 35 26, 50 10 C 65 5, 72 12, 80 2');

  return `
    <div class="kpi-card ${color}">
      <div class="kpi-card-top">
        <div class="kpi-icon ${color}">
          ${KPI_ICONS[icon] || KPI_ICONS.money}
        </div>
        ${deltaHtml}
      </div>
      <div class="kpi-label">${label}</div>
      <div class="kpi-value" data-animate>${displayValue}</div>
      ${sub ? `<div class="kpi-sub">${sub}</div>` : ''}
      <svg class="kpi-sparkline" viewBox="0 0 80 30">
        <path d="${sparklinePath}" />
      </svg>
    </div>
  `;
}

export function renderKPIGrid(cards) {
  const isSeven = cards.length === 7;
  return `<div class="${isSeven ? 'kpi-grid-7' : 'kpi-grid'}">${cards.map(c => renderKPICard(c)).join('')}</div>`;
}
